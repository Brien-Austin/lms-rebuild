...
const Owned = ({title,imageUrl,isFree,hasOwned,description,userId,email,chapters,price,firstName,  category,courseId,totalChapters} : OwnedProps) => {
  ...
  return (
    ...
    {imageUrl !== null && (
      <div className='relative h-36 w-full'>
        <h1 className='text-xs top-0 left-0 absolute z-10 border border-indigo-600 bg-white p-1 rounded text-indigo-600' style={{ textShadow: '2px 2px 3px rgba(0, 0, 0, 0.1)' }}>
          {category}
        </h1>
    ...
    <div className='flex-grow'>
      <div className='flex justify-between items-center'>
        <h1 className='text-md pt-2 pb-1'>{title}</h1>
      </div>
      <h1 className='text-muted-foreground italic text-xs truncate'>{description}</h1>
    ...
    <div className='flex justify-between items-center'>
      {isFree ? (
        <>
          ...
          {hasOwned ? (
            <Link href={`/course/${courseId}`}>
              <Button variant={'purchase'}>Continue Learning</Button>
            </Link>
          ) : (
        ...
        <>
          ...
          <Button variant={'purchase'}>Buy course</Button>
        </>
      )}
    </div>
  </Link>
